  ...
  if (pkg %in% remotes_packages) {
    ...
    if (!file.exists(file.path("repo", remote_target))) {
      ...
      make_remote_tarball(
        ...
        gsub(x=remote_target, pattern= paste0("_",substr(remote_info$extra[[1]]$remotesha,1,7)),replacement="")
      )
    }

    tarball_file <- basename(gsub(x=remote_target, pattern= paste0("_",substr(remote_info$extra[[1]]$remotesha,1,7)),replacement=""))
